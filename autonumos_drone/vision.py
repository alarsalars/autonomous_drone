...
def draw_system_status(frame,
                       ...
                       offboard_ok: bool):
    """
    Top-left: PX4 + mode + sats + alt + battery + armed + Offboard status.
    """
    status = "LIVE" if flight_on else "DRYRUN"
    arm_text = "ARMED" if armed else "DISARMED"

    if not flight_on:
        off_text = "OFFBOARD IDLE"
        off_color = (200, 200, 200)   # gray in DRYRUN
    else:
        if offboard_ok:
            off_text = "OFFBOARD OK"
            off_color = (0, 255, 0)   # green
        else:
            off_text = "OFFBOARD LOST"
            off_color = (0, 165, 255) # orange-ish

    # Line 1: general + Offboard state
    line1 = f"PX4 | {status} | {arm_text} | {off_text}"
    # Line 2: detailed telemetry
    line2 = f"MODE: {mode} | SATS: {sats} | ALT: {alt_m} m | BAT: {bat_str}"

    # Draw line1 with Offboard color, line2 white
    frame = put_text_outline(frame, line1, (10, 22), scale=0.6,
                             color=off_color, thickness=1)
