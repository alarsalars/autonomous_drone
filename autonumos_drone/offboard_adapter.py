...
class VelocityController:
    ...
    def __init__(self,
                 ...
                 safety: SafetyConfig,
                 backend: str = "print",
                 telemetry: Px4TelemetryClient | None = None):
        self.safety = safety
        ...
        if b == "ardupilot":
            ...
            # If you still have a Px4MavlinkBackend, leave this branch as-is
            self.backend = Px4MavlinkBackend(safety)
        elif b == "mavsdk":
            if telemetry is None:
                raise ValueError("MAVSDK backend requires a Px4TelemetryClient instance")
            self.backend = MavsdkBackend(safety, telemetry)
        else:
...
class MavsdkBackend:
    """
    PX4 MAVSDK backend:
      - Uses Px4TelemetryClient to send Offboard velocity commands
      - Respects SafetyConfig.ENABLE_FLIGHT:
          * False -> DRYRUN (prints only)
          * True  -> sends real MAVSDK offboard setpoints
    """

    def __init__(self, safety: SafetyConfig, tele: Px4TelemetryClient):
        self.cfg = safety
        self.tele = tele

    def start(self) -> None:
        print("[Adapter] MAVSDK backend ready.")
        # We will start Offboard lazily on first send() when ENABLE_FLIGHT=True


    def stop(self) -> None:
        if self.cfg.ENABLE_FLIGHT:
            self.tele.stop_offboard()
        print("[Adapter] MAVSDK backend stopped.")

    def send(self, sp: Setpoint) -> None:
        # DRYRUN: show what we *would* send
        if not self.cfg.ENABLE_FLIGHT:
            print(
                f"[DRYRUN][MAVSDK] vx={sp.vx:+.2f} m/s  vy={sp.vy:+.2f}  "
                f"vz={sp.vz:+.2f}  yaw_rate={sp.yaw_rate:+.2f} rad/s"
            )
            return

        # LIVE: ensure Offboard is started, then send velocity setpoint
        self.tele.ensure_offboard_started()
        self.tele.send_offboard_velocity(sp.vx, sp.vy, sp.vz, sp.yaw_rate)
