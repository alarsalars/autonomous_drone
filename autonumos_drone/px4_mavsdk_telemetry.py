...
class Px4TelemetryClient:
    ...
    def __init__(self, serial: str = "/dev/ttyTHS1", baud: int = 57600):
        ...
        self._last_offboard_cmd: float = 0.0   # NEW: last time we sent a velocity
    ...
    def get_state(self) -> TelemetryState:
        now = time.time()
        with self._lock:
            # Offboard considered OK if:
            #  - we've started Offboard
            #  - AND we sent a velocity in the last ~1 second
            offboard_ok = self._offboard_started and (now - self._last_offboard_cmd < 1.0)
    ...
    async def _ensure_offboard_started(self):
        ...
        try:
            for _ in range(5):
                # Your MAVSDK version wants positional args, not keywords
                vel = VelocityBodyYawspeed(0.0, 0.0, 0.0, 0.0)
                await self._system.offboard.set_velocity_body(vel)
                await asyncio.sleep(0.1)
    ...
    async def _offboard_set_velocity(self, vx: float, vy: float, vz: float, yaw_rate: float):
        ...
        try:
            # Positional: (vx_m_s, vy_m_s, vz_m_s, yaw_rate_rad_s)
            vel = VelocityBodyYawspeed(vx, vy, vz, yaw_rate)
            await self._system.offboard.set_velocity_body(vel)


            # NEW: remember last time we sent something
            self._last_offboard_cmd = time.time()
    ...
    async def _offboard_stop(self):
        ...
        try:
            ...
            self._last_offboard_cmd = 0.0     # NEW
